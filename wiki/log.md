...
## 날짜별 로그
- [[2026-06-26]] — [[AGENTS.md 작성 가이드]] 위키 부트스트랩 방식 전환: `FRONTEND_LLM_WIKI_ROOT`+symlink resolver를 `$HOME/.llm-wiki` 단일 위치 부트스트랩(셸별 홈 절대경로 → 없으면 1회 clone·있으면 `pull --ff-only` → 절대경로로 index 열기)으로 교체 → `0.6.1`
