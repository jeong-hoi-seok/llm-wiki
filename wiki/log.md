...
## 날짜별 로그
- [[2026-06-29]] — 위키 연동 방식 절대경로 → 심볼릭링크 전환: 소비 프로젝트가 `.llm-wiki` 심볼릭링크로 `$HOME/.llm-wiki`를 가리켜 상대경로 `./.llm-wiki/...`로 읽고, 링크는 `.gitignore`로 제외·머신마다 재생성. `AGENTS.md`에 `프로젝트 연동` 섹션 신설(본진 push / 소비 링크 커밋 금지 구분), [[AGENTS.md 작성 가이드]] 심볼릭링크로 전면 재작성(SSOT). 중복 정합: 본진/가이드 템플릿 복붙 제거·링크화, [[LLM Wiki와 옵시디언으로 프론트엔드 작업 자동화하기]] 현행화 노트, [[위키 개요]] SSOT 콜아웃 → `0.8.0`
