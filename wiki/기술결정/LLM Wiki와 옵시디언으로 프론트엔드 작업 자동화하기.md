...
> [추론] **연결 방식 현행화 (2026-06-29):** 위 "AGENTS.md에 git 경로를 추가해 참조"는 초기 설계다. 이후 절대경로 하드코딩 문제(머신마다 깨짐)를 없애기 위해, 위키 실체는 `$HOME/.llm-wiki` 한 곳에 두고 각 프로젝트는 `.llm-wiki` **심볼릭링크**로 연결하는 방식으로 바뀌었다. 링크는 `.gitignore`로 제외하고 머신마다 부트스트랩으로 재생성한다. 연결·셋업 절차의 단일 기준은 [[AGENTS.md 작성 가이드]].
