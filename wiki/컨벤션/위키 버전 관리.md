...
위키 버전은 `wiki/index.md` frontmatter의 `version` 값으로 관리한다. 형식은 SemVer를 빌려온다.
...
버전 작업 전, GitHub Release 최신 tag와 `wiki/index.md`의 `version`이 같은지 확인한다. tag는 `vX.Y.Z`, version은 `X.Y.Z`이므로 `v${version}`으로 맞춰 본다.
...
`origin/main`의 `wiki/index.md`가 더 최신이면 작업 전 갱신한다.
...
1. 변경 범위를 확인한다.
2. GitHub Release 최신 tag와 `wiki/index.md`의 `version`을 비교한다. 불일치 시 위 [원격 버전 동기화](#원격-버전-동기화)를 따른다.
3. 이 문서 기준으로 `minor` / `patch`를 고른다.
4. `wiki/index.md`의 `version`과 `updated`를 갱신한다.
5. `wiki/log/YYYY-MM-DD.md`에 이전 버전·새 버전·bump 이유를 남긴다.
6. `wiki/log.md` 날짜별 요약도 갱신한다.
...
- `wiki/index.md`가 핵심 문서를 빠짐없이 가리킴.
