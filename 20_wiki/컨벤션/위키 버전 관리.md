...
위키 버전은 `20_wiki/index.md` frontmatter의 `version` 값으로 관리한다. 형식은 SemVer를 빌려온다.
...
버전 작업 전, GitHub Release 최신 tag와 `20_wiki/index.md`의 `version`이 같은지 확인한다. tag는 `vX.Y.Z`, version은 `X.Y.Z`이므로 `v${version}`으로 맞춰 본다.
...
`origin/main`의 `20_wiki/index.md`가 더 최신이면 작업 전 갱신한다.

```sh
git pull --ff-only origin main
```

로컬 변경이 있거나 fast-forward가 안 되면 **임의로 reset/rebase하지 않는다.** 현재 `index.version`, GitHub Release tag, `origin/main`의 `index.version`을 확인해 동기화 필요 상태를 사용자에게 먼저 보고한다.

## 작업 절차

1. 변경 범위를 확인한다.
2. GitHub Release 최신 tag와 `20_wiki/index.md`의 `version`을 비교한다. 불일치 시 위 [원격 버전 동기화](#원격-버전-동기화)를 따른다.
3. 이 문서 기준으로 `minor` / `patch`를 고른다.
4. `20_wiki/index.md`의 `version`과 `updated`를 갱신한다.
5. `20_wiki/log/YYYY-MM-DD.md`에 이전 버전·새 버전·bump 이유를 남긴다.
6. `20_wiki/log.md` 날짜별 요약도 갱신한다.
7. 작업 브랜치에 커밋 → push → PR 생성([[MR PR 작성 가이드]] 위키 PR 템플릿). `main` 직접 커밋 금지.
8. PR은 **반드시 squash merge**한다. 머지로 `main`이 갱신된 뒤 `vX.Y.Z` tag 생성(`v` + index의 `version`) → tag push.
9. GitHub Release를 만든다. 제목은 tag와 같게, 본문에는 해당 날짜 log의 버전 변경 요약을 압축해 적는다.

## 1.0.0 기준

아래를 만족하면 `1.0.0` 검토 가능.

- `AGENTS.md`와 주요 `AGENTS.md 작성 가이드`가 팀 공유 방식까지 포함.
- `20_wiki/index.md`가 핵심 문서를 빠짐없이 가리킴.
