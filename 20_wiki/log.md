...
작업 이력 색인. 실제 ingest / query / lint 기록은 `20_wiki/log/YYYY-MM-DD.md`에 일자별로 남긴다.

## 작성 규칙
- 새 작업 기록은 해당 날짜 파일에 append한다.
- 날짜 파일이 없으면 `20_wiki/log/YYYY-MM-DD.md`를 만든다.
- `log.md`에는 날짜별 링크와 간단한 요약만 남긴다.
- `index.md`와 운영 문서가 바뀐 작업은 날짜 로그에도 함께 기록한다.

## 날짜별 로그
- [[2026-06-29]] — 위키 연동 방식 절대경로 → 심볼릭링크 전환: 소비 프로젝트가 `.llm-wiki` 심볼릭링크로 `$HOME/.llm-wiki`를 가리켜 상대경로 `./.llm-wiki/...`로 읽고, 링크는 `.gitignore`로 제외·머신마다 재생성. `AGENTS.md`에 `프로젝트 연동` 섹션 신설(본진 push / 소비 링크 커밋 금지 구분), [[AGENTS.md 작성 가이드]] 심볼릭링크로 전면 재작성(SSOT). 중복 정합: 본진/가이드 템플릿 복붙 제거·링크화, [[LLM Wiki와 옵시디언으로 프론트엔드 작업 자동화하기]] 현행화 노트, [[위키 개요]] SSOT 콜아웃 → `0.8.0`, 위키 변경 워크플로 브랜치·PR 필수화(`main` 직접 커밋 금지, [[MR PR 작성 가이드]] 위키 PR 템플릿 추가, tag·Release는 머지 후) → `0.9.0`, [[LLM Wiki와 옵시디언으로 프론트엔드 작업 자동화하기]] LinkedIn 발행본 기준 전면 재작성(공통 Store 분리·심볼릭링크·경로 대안 비교, status verified) → `0.9.1`, 루트 폴더 번호 프리픽스(`00_context`·`10_raw`·`20_wiki`, 진입경로 `./.llm-wiki/20_wiki/index.md`, 링크명 불변) + `.obsidian/` 통째 gitignore·추적 해제 → `0.10.0`, `.obsidian` 통째 ignore 되돌려 공유 설정(플러그인·테마) 재추적·workspace/cache만 제외 → `0.10.1`, 링크 감사 후 [[Hermes]]의 깨진 `[[New Architecture]]`를 `[[React Native#New Architecture]]` 섹션 링크로 수정 → `0.10.2`, 부트스트랩 존재 체크 경로 버그 수정(`.llm-wiki/wiki` → `.llm-wiki/20_wiki`, 0.10.0 폴더 번호 작업 누락분) → `0.10.3`
- [[2026-06-26]] — [[AGENTS.md 작성 가이드]] 위키 부트스트랩 방식 전환: `FRONTEND_LLM_WIKI_ROOT`+symlink resolver를 `$HOME/.llm-wiki` 단일 위치 부트스트랩(셸별 홈 절대경로 → 없으면 1회 clone·있으면 `pull --ff-only` → 절대경로로 index 열기)으로 교체 → `0.6.1`, [[위키 버전 관리]] 가독성 정리(역사적 섹션 삭제·표 슬림화 132→109행) + [[index]]에 원격 동기화 "최우선 체크" 콜아웃 추가(필수 선행 체크 격상) → `0.7.0`, [[위키 버전 관리]] 작업 절차의 tag 예시 `v0.6.0` 제거(플레이스홀더화) → `0.7.1`
